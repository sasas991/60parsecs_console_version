    ...
    public static gameevent createRandomEvent() {
        ...
        switch (eventType) {
            case 0: return (state, ui) -> {
                ui.println("📡 СОБЫТИЕ: Метеоритный дождь!");
                ...
                if (choice == 1) {
                    if (state.hasItem("Лазерный пистолет")) {
                        ui.println("🔫 Пиу-пиу! Метеориты уничтожены.");
                    } else {
                ...
                if (choice == 2) {
                    if (random.nextBoolean()) {
                        ui.println("✅ Отличный маневр! Урон избежан.");
                    } else {
                        ui.println("❌ Не удалось уклониться. Корпус задет.");
                        state.ship -= 10;
                        ui.println("   Корпус -10%");
                    }
                } else if (choice == 3) {
                    state.ship -= 15;
                    ui.println("💥 Удар принят. Корпус -15%");
                }
            };
            case 1: return (state, ui) -> {
                ui.println("📡 СОБЫТИЕ: Дрейфующий контейнер.");
                ...
                if (choice == 1) {
                    if (random.nextInt(100) < 70) {
                        state.food += 20;
                        ui.println("📦 Внутри оказалась еда! +20%");
                    } else {
                        state.ship -= 10;
                        ui.println("💥 Это была мина! Корпус -10%");
                    }
                } else if (choice == 2) {
                    if (state.crew.contains("Учёный Макс")) {
                        ui.println("🔬 Макс определил, что контейнер безопасен.");
                        state.food += 20;
                        ui.println("   Еда +20%");
                    } else {
            ...
            case 2: return (state, ui) -> {
                 ui.println("📡 СОБЫТИЕ: Торговец из глубокого космоса.");
            ...
            case 3: return (state, ui) -> {
                ui.println("📡 СОБЫТИЕ: Космическая пыль на фильтрах.");
                ...
                if (input.equalsIgnoreCase("ЧИСТКА") && (end - start) < 4000) {
                    ui.println("✅ Фильтры чисты. Кислород в норме.");
                } else {
                    ui.println("❌ Слишком медленно или ошибка! Фильтры забились.");
                    state.oxygen -= 10;
            ...
            case 4: return (state, ui) -> {
                ...
                if (choice == 1) {
                    if (state.hasItem("Аптечка")) {
                        ui.println("💊 Аптечка использована. Экипаж здоров.");
                ...
                if (choice == 2) {
                    if (state.food >= 10) {
                        state.food -= 10;
                        if (random.nextBoolean()) {
                            ui.println("🍵 Горячий суп помог! Экипаж здоров.");
                        } else {
                            ui.println("💀 Не помогло. Член экипажа погиб.");
                ...
                if (choice == 3) {
                    ui.println("💀 Болезнь взяла свое. Член экипажа погиб.");
                    if (!state.crew.isEmpty()) state.crew.remove(0);
                }
            };
